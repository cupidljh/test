...
extern void hdlc_enc_init(hdlc_enc *hdlc_encode); //hdlc 구조체를 초기화 해준다.
extern void hdlc_dec_init(hdlc_dec *hdlc_decode);
extern void hdlc_encode(char *buf, size_t buflen,  hdlc_enc *hdlc_encode); // 파일에서 16byte씩 데이터를 가져온다.
extern void hdlc_decode(hdlc_enc *hdlc_encode, hdlc_dec *hdlc_decode);// decode
extern unsigned short compute_fcs(unsigned char *data, int length); //  CRC값 계산
